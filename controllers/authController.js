      ...
      user.find({username: request.payload.username, password: password}, function(err, user){
          ...
          if(!err){
            if(user.length > 0){
              ...
              return reply({foto : user[0].foto, username: user[0].username, scope: user[0].scope, password: user[0].password, phone: user[0].phone, name: user[0].name, email: user[0].email});
            }
